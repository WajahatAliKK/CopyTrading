from bot.utils.wallet_methods import eth_test_wm, eth_uni_m, eth_uni_mv3, arb_uni_m, arb_wm, bsc_uni_m, bsc_wm
...
def get_pair_price(token_in, token_out, dex):
    # logger.info(f"{token_in}, {token_out}, {dex}")
    try:
        if "uni" in dex.lower() or "":
            router = uniswap_base
        elif "pan" in dex.lower():
            router = pancakeswap_base
        else:
        ...
        if token_out==ETH_ADDRESS:
            ...
            token_out, token_in = token_in, token_out
...
async def sell_trade(user: User, trade: ActiveTrades, wallet: Wallet):
    try:
        ...
        # resp = own_dex.approve(trade.token_address,balance)
        # logger.info(f"Approval Status: {resp}")
        status, hsah = own_dex.swap_token_to_eth(balance, min_out_amount, [trade.token_address, token_out], own_dex.wallet, deadline_seconds=60, gas_delta=user_setting.max_gas_price)
