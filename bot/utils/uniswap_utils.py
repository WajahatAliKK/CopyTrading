...
from bot.utils.config import WETH_ADDRESS, WETH_ADDRESS_ARB, WBNB_ADDRESS
import logging
...
class UniswapUtils:
    ...
    def approve(self, token_address, amount):
        ...
        transaction = {
            ...
            'gas': gas,
            'gasPrice': gas_price,
            'nonce': self.web3.eth.get_transaction_count(self.wallet, 'pending'),
            'data': data,
        }
    ...
    def swap_eth_for_usdt(self,usdt_address, usdt_amount):
        ...
        # Contract addresses
            ...
            signed_tx = self.web3.eth.account.sign_transaction(transaction, self.private_key)
            # txn = uniswap_router.functions.swapETHForExactTokens(usdt_amount, [weth_address, usdt_address], self.wallet, deadline).build_transaction({
            #     'value': eth_amount,
            #     'chainId': self.web3.eth.chain_id,
            #     'gas': 150000,
